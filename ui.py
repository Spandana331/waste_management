...
ml=load_model("D:\\waste_management\\DATASET\\waste_management.h5")
...
tab1,tab2,tab3=st.tabs(["🎦Upload video","📷Webcam","🗃️chatbot"])

with tab1:
    st.header("Upload the video🎥 ")
    upl=st.file_uploader("Upload the video ",type=["mp4","mku","avi","mov"])
    prd=[]
    if upl:
